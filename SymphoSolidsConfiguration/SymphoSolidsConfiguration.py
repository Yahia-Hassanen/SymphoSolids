...
root.after(201, lambda: root.iconbitmap('C:/Users/1yahi/Desktop/Dr Chan Summer 24/Musical instruments/Accessory Code/LOGO.ico'))
...
dot_product_vars = [ctk.StringVar() for _ in faces]
...
def confirm_sides():
   ...
   debounce_timer = root.after(3000, lambda: [update_face_buttons_and_entries(sides)])
        ...
        with open(file_path, 'w') as file:
            ...
            def write_entries(entries, mode):
                for i, face in enumerate(faces):
                    for label in ['Note', 'X', 'Y', 'Z', 'Dot Product']:
                        entry = entries[label][i]
                        if label == 'Dot Product':
                            value = entry.cget("text")  # Get text from label
                        else:
                            value = entry.get()  # Get text from entry
                        file.write(f"#define {mode}_{face}_{label.upper()} {value}\n")
...
# Add the Tabview widget instead of a single frame for face management
...
def create_face_layout(parent_frame, entries):
    ...
    for row_index, label in enumerate(['Note', 'X', 'Y', 'Z', 'Dot Product'], start=2):
        ctk.CTkLabel(master=parent_frame, text=label, font=('Bahnschrift SemiBold', 12)).grid(row=row_index, column=0, padx=5, pady=5, sticky='nsew')
        for col_index in range(1, 11):
            if label == 'Dot Product':
                ctk.CTkLabel(master=parent_frame, textvariable=dot_product_vars[col_index - 1], font=('Bahnschrift SemiBold', 12)).grid(row=row_index, column=col_index, padx=5, pady=5, sticky='nsew')
            elif label in ['X', 'Y', 'Z']:
    ...
    for row_index, label in enumerate(['Note', 'X', 'Y', 'Z', 'Dot Product'], start=2):
        ctk.CTkLabel(master=parent_frame, text=label, font=('Bahnschrift SemiBold', 12)).grid(row=row_index + 6, column=0, padx=5, pady=5, sticky='nsew')
        for col_index in range(11, 21):
            if label == 'Dot Product':
                ctk.CTkLabel(master=parent_frame, textvariable=dot_product_vars[col_index - 1], font=('Bahnschrift SemiBold', 12)).grid(row=row_index + 6, column=col_index - 10, padx=5, pady=5, sticky='nsew')
            elif label in ['X', 'Y', 'Z']:
                entry = ctk.CTkLabel(master=parent_frame, text="0", font=('Bahnschrift SemiBold', 12))
...
button_scan = ctk.CTkButton(f_device_manager, height=30, width=270, text="Scan for Devices", command=lambda: asyncio.run_coroutine_threadsafe(scan_devices(), loop),
...
device_menu = ctk.CTkOptionMenu(f_device_manager, height=30, width=270, variable=selected_address, values=options, fg_color='#c7c7cc', text_color="#302c2c")
device_menu.grid(row=2, column=0, padx=20, pady=5)


button_connect = ctk.CTkButton(f_device_manager, text="Connect", height=30, width=270, command=confirm_selection, fg_color='#6ac4dc', hover_color='#24cc44', text_color="#302c2c")
button_connect.grid(row=3, column=0, padx=20, pady=5)


button_disconnect = ctk.CTkButton(f_device_manager, text="Disconnect", height=30, width=270, command=disconnect, fg_color='#6ac4dc', hover_color='#ff375f', text_color="#302c2c")
...
label_2 = ctk.CTkLabel(master=f_name, text='Name Device', anchor='center', font=('Bahnschrift SemiBold', 16), text_color="#302c2c")
...
sides_entry = ctk.CTkLabel(f_sides, textvariable=number_of_sides, font=('Bahnschrift SemiBold', 27), text_color="#302c2c")
...
button_save = ctk.CTkButton(master=f_file, height=30, width=280, text="Save to File", fg_color='#6ac4dc', hover_color='#008299', text_color="#302c2c", command=save_to_file)
button_save.grid(row=0, column=0, padx=10, pady=5)


button_import = ctk.CTkButton(master=f_file, height=30, width=280, text="Import File", fg_color='#6ac4dc', hover_color='#008299', text_color="#302c2c", command=import_file)
...
entries = {label: [] for label in row_labels}
